...
import { Users, Heart, Home, BookOpen, Moon, Sparkles, AlertCircle, MapPin, Clock, Home as HomeIcon, CheckCircle, Brain } from 'lucide-react'
...
export default function RoomieUser() {
  ...
  const [resultsReleased, setResultsReleased] = useState(false) // Admin controls this
  const [batchStatus, setBatchStatus] = useState<{
    isComplete: boolean
    resultsAvailable: boolean
    progress: string // e.g., "Most students", "Almost there", "Collecting responses"
  }>({
    isComplete: false,
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        // Check if results are released (simulate checking admin setting)
        // In real implementation, this would come from Firebase
        setResultsReleased(false) // Admin controls this
  ...
  const handleSurveySubmit = async (formData: IndianSurveyFormData) => {
    console.log('handleSurveySubmit called with:', formData)
    console.log('User UID:', user?.uid)
    console.log('Current Student:', currentStudent)
    
    // Temporarily allow submission without user for testing
    ...
    try {
      ...
      // Instead of showing matches immediately, show confirmation
      setExistingSurvey(survey)
      setShowMatches(false) // Don't show matches until admin releases
      
      // Update batch status (simulate)
      setBatchStatus(prev => ({
        ...prev,
        progress: "Response recorded",
        isComplete: false // Admin controls when it's complete
      }))
  ...
  */
  ...
  // Show results only if survey is submitted AND results are released
  if (existingSurvey && !resultsReleased) {
    return (
      <div className="max-w-4xl mx-auto p-6 space-y-6">
        <div className="bg-gradient-to-r from-blue-50 to-green-50 border border-blue-200 rounded-xl p-8 text-center">
          ...
          <h2 className="text-2xl font-bold text-gray-900 mb-4">Survey Submitted Successfully! 🎉</h2>
          <p className="text-lg text-gray-700 mb-6">
            Thank you for completing the Indian Hostel Roommate Survey
          </p>
          
          {/* Batch Progress */}
          <div className="bg-white border border-gray-200 rounded-xl p-6 mb-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">Waiting for All Students</h3>
            <div className="space-y-4">
              ...
              <div className="w-full bg-gray-200 rounded-full h-3">
                <div 
                  className="bg-gradient-to-r from-blue-500 to-green-500 h-3 rounded-full transition-all duration-300 animate-pulse"
                  style={{ width: `${batchStatus.isComplete ? '100' : '60'}%` }}
                ></div>
              </div>
              <p className="text-sm text-gray-600">
                {batchStatus.isComplete 
                  ? "🎯 All surveys collected! Results are being processed..." 
                  : "⏳ Waiting for more students to complete their surveys"
                }
              </p>
            </div>
          </div>
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-6 text-left">
            ...
            <div className="space-y-3">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="font-medium text-gray-900">1. Wait for all surveys</p>
                  <p className="text-sm text-gray-600">We need all students to complete their surveys</p>
                </div>
              </div>
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="font-medium text-gray-900">2. Process matches</p>
                  <p className="text-sm text-gray-600">Our algorithm will find your best roommate matches</p>
                </div>
              </div>
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="font-medium text-gray-900">3. Results released</p>
                  <p className="text-sm text-gray-600">You'll be notified when your roommate matches are ready!</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="mt-6 p-4 bg-gray-50 rounded-lg">
            <p className="text-sm text-gray-600">
              Questions? Contact hostel administration or check back here for updates.
            </p>
          </div>
        </div>
      </div>
